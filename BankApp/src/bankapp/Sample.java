...
public class Sample extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        /* This is the dropDown menu that allows to user to choose which
            actions he/she wants to perform
         */
        ComboBox<String> dropMenu = new ComboBox<String>();
        ComboBox<String> dropMenu2 = new ComboBox<String>();
        
        // These are the options the user has to choose from      
        ...
        ObservableList<String> banks = FXCollections.observableArrayList("Citi", "Chase", "American Express");
        dropMenu.getItems().addAll(banks);
        dropMenu.setId("dropMenu");
        dropMenu.setPrefSize(1050, 30); 
        
       
        
         Label lbl = new Label("Select A Service: ");        
        
        BorderPane paneForDropMenu = new BorderPane();
        paneForDropMenu.setLeft(lbl);
        paneForDropMenu.setCenter(dropMenu);
        
        borderpane.setTop(paneForDropMenu);
        ...
        dropMenu.setOnAction(e -> {
            if (dropMenu.getValue().equals("Open New Account")) {
                openAcFunction();
            } else if (dropMenu.getValue().equals("Open CD Account")) {
                opencdFunction();
            } else if (dropMenu.getValue().equals("Deposit")) {
                dpositFunction();
            } else if (dropMenu.getValue().equals("Withdraw")) {
                withdrawFunction();
            } else if (dropMenu.getValue().equals("Check Balance")) {
                checkBalanceFunction();
            } else if (dropMenu.getValue().equals("Close Account")) {
        ...
        Scene scene = new Scene(borderpane, 975, 525);
        
        primaryStage.setScene(scene);
        scene.getStylesheets().add(ComboProj.class.getResource("BankApp.css").toExternalForm());
        primaryStage.show();
    }
